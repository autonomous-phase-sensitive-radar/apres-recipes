...
%% Fixing the antenna index
                % Reza Ershadi
                tx1 = find(vdat.TxAnt==1);
                tx2 = find(vdat.TxAnt>1);
                rx1 = find(vdat.RxAnt==1);
                rx2 = find(vdat.RxAnt>1);
                if ~isempty(tx2)
                    if length(tx1) == length(rx1)
                        vdat.TxAnt(tx2) = 0;
                    else
                        vdat.TxAnt(tx2) = 1;
                    end
                end
                tx0 = find(vdat.TxAnt==0);
                rx0 = find(vdat.RxAnt==0);
                vdat.TxAnt(tx0) = [];
                vdat.RxAnt(rx0) = [];
%% ------------------------------------------------- 
                if vdat.Average
