%% Comparing 2 consecutive bursts
close all;
%[range,dh,dhe,dt,c] = fmcw_melt('data2022/DATA2022-06-05-1036.DAT','data2022/DATA2022-06-05-1036.DAT',46,47,0);

%[range,dh,dhe,dt,c] = fmcw_melt('data2022/DATA2022-09-18-0054.DAT','data2022/DATA2022-09-18-0054.DAT',4,5,0);

[range,dh,dhe,dt,c] = fmcw_melt('data2022/DATA2022-06-05-1036.DAT','data2022/DATA2022-09-18-0054.DAT',47,4,0);
...
if numel(attSetList)>1
    disp(['Warning: multiple attenuator settings found file ' name1 '. Keeping second set only.'])
    fs = fmcw_burst_split_by_att(f);
    f = fs(2); % keeing results from second.
end
attSetList = unique(g.chirpAtt,'stable');
if numel(attSetList)>1
    disp(['Warning: multiple attenuator settings found file ' name2 '. Keeping second set only.'])
